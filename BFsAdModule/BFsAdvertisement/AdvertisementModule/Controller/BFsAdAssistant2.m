...
@interface BFsAdAssistant2 ()

@end

@implementation BFsAdAssistant2

#pragma mark - API

static BFsAdAssistant2 *assistant;
static dispatch_once_t onceToken;

+ (instancetype)shareAssistant {
    
    dispatch_once(&onceToken, ^{
        assistant = [[BFsAdAssistant2 alloc] init];
    });
...
- (void)requireAdvertisementInfoFromUrl:(NSString *)pUrlStr andResultBlock:(returnBlock)block {
    ...
    [self requireAdvertisementDataFromUrl:pUrlStr andResultBlock:^(NSError *error, id result) {
        if (error) {
            ...
            if (modes) {
                [self downloadResourcesFromNetwork:modes andResultBlock:^(id result, NSError *error, BFsResultType type) {
...
- (id)loadBrandAdvertisements {
    ...
    NSString *adBrand = [[NSUserDefaults standardUserDefaults] objectForKey:kBrandKey];
    
    return [self loadBrandAdvertisement:adBrand];
}

- (void)setBrand:(NSString *)brand {
    
    if (brand.length > 0) {
        [[NSUserDefaults standardUserDefaults] setObject:brand forKey:kBrandKey];
    }
}
...
- (void)downloadResourcesFromNetwork:(id)modes andResultBlock:(resultBlock2)block {
    ...
    for (BFsBrandMode *mode in modes) {
        ...
        [self addAsynchronousDownloadTaskInGroup:downloadGroup queue:downloadQueue url:mode.advertDown1 brand:mode.brand localFileName:@"advertDown1" andResultBlock:^(id result, NSError *error, BFsResultType type)  {
            //
        ...
        [self addAsynchronousDownloadTaskInGroup:downloadGroup queue:downloadQueue url:mode.advertDown2 brand:mode.brand localFileName:@"advertDown2" andResultBlock:^(id result, NSError *error, BFsResultType type) {
            //
        ...
        [self addAsynchronousDownloadTaskInGroup:downloadGroup queue:downloadQueue url:mode.advertDown3 brand:mode.brand localFileName:@"advertDown3" andResultBlock:^(id result, NSError *error, BFsResultType type) {
            //
        ...
        [self addAsynchronousDownloadTaskInGroup:downloadGroup queue:downloadQueue url:mode.iconDown brand:mode.brand localFileName:@"iconDown" andResultBlock:^(id result, NSError *error, BFsResultType type) {
            //
...
- (void)moveFilesFromTempToTargetDirectory:(id)modes andResultBlock:(resultBlock2)block {
    ...
    for (BFsBrandMode *mode in modes) {
        ...
        [self addAsynchronousMoveTaskInGroup:moveGroup queue:moveQueue url:mode.advertDown1 brand:mode.brand localFileName:@"advertDown1" andResultBlock:^(id result, NSError *error, BFsResultType type) {
            //
        ...
        [self addAsynchronousMoveTaskInGroup:moveGroup queue:moveQueue url:mode.advertDown2 brand:mode.brand localFileName:@"advertDown2" andResultBlock:^(id result, NSError *error, BFsResultType type) {
            //
        ...
        [self addAsynchronousMoveTaskInGroup:moveGroup queue:moveQueue url:mode.advertDown3 brand:mode.brand localFileName:@"advertDown3" andResultBlock:^(id result, NSError *error, BFsResultType type) {
            //
        ...
        [self addAsynchronousMoveTaskInGroup:moveGroup queue:moveQueue url:mode.iconDown brand:mode.brand localFileName:@"iconDown" andResultBlock:^(id result, NSError *error, BFsResultType type) {
            //
...
- (void)saveToLocalImage:(UIImage *)image name:(NSString *)localFileName brand:(NSString *)brand
    ...
    BFsResultType resType = result ? ResultTypeDefaultSuccess : ResultTypeDefaultFail;
